...
public:
  ...
  OscillatoreArmonico(double omega0){m_omega0 = omega0; }; //costruttore
...
//classe pendolo reale
class PendoloReale : public FunzioneVettorialeBase {
public:

  PendoloReale(double omega0){m_omega0 = omega0; }; //costruttore

	virtual vector<double> Eval(double t,const vector<double> &x)const{
		vector<double> dxdt {x[1],(-(g/l)*sin(x[0]))};
	return dxdt;
  }
