...
class OscillatoreArmonicoForzante : public FunzioneVettorialeBase {
public:

  OscillatoreArmonicoForzante(double omega0,double alpha,double A, double omega){
	  m_omega0 = omega0; 
	  m_alpha = alpha;
	  m_A = A;
	  m_omega = omega;
	...
	virtual vector<double> Eval(double t,const vector<double> &x)const{
		vector<double> dxdt {x[1],(-m_omega0*m_omega0*x[0])+sin(m_omega*t)-m_alpha*x[1] };
	return dxdt;
